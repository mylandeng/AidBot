...
flowchart LR
  ...
  P6 --- P6D[多轮规划搜索 / 意图识别 / Query 改写 / 知识库路由]
```
...
> 2026-07-07 路线备注：基于大模型的 Agentic RAG（多轮规划搜索、自动意图识别、Query 改写、知识库路由后再生成回答）不属于当前 MVP 范围，只作为平台化之后的高级检索编排演进项。
