...
- 已完成统一错误处理第一版：HTTP/SSE 共用结构化错误载荷和请求 ID，保留旧 `detail` 兼容；聊天失败、超时和用户主动停止均在对应回答位置显示状态，管理员可查看错误码和请求 ID。
- 已新增错误合同测试，覆盖登录失败、参数校验、检索不可用和模型异常，确认内部异常文本不会返回客户端。
- 最终验证：后端测试 62/62 通过，前端 `npm run typecheck` 与 `npm run build` 通过，并完成管理员聊天断网失败提示及再次提问的浏览器回归。
