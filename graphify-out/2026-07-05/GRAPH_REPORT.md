...
## Corpus Check
- 70 files · ~6,822 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 346 nodes · 520 edges · 45 communities (23 shown, 22 thin omitted)
- Extraction: 78% EXTRACTED · 22% INFERRED · 0% AMBIGUOUS · INFERRED: 115 edges (avg confidence: 0.52)
...
1. `CurrentUser` - 26 edges
2. `ChatRequest` - 23 edges
3. `ChatService` - 18 edges
4. `ChatResponse` - 16 edges
5. `compilerOptions` - 16 edges
6. `Conversation` - 15 edges
7. `Message` - 15 edges
8. `FastAPI` - 13 edges
...
Cohesion: 0.23
Nodes (25): create_chat_response(), stream_chat_response(), get_conversation(), list_conversations(), CurrentUser, ChatRequest, ChatResponse, CurrentUser (+17 more)
...
Cohesion: 0.08
Nodes (7): create_app(), lifespan(), FastAPI, auth_headers(), test_chat_can_continue_existing_conversation(), test_chat_persists_conversation_and_placeholder_sources(), test_stream_chat_emits_delta_and_structured_final()
...
Cohesion: 0.16
Nodes (20): apiBaseUrl(), askQuestion(), askQuestionStream(), authorized(), getConversation(), listConversations(), login(), parseStreamEvent() (+12 more)
...
Cohesion: 0.24
Nodes (6): Base, datetime, DeclarativeBase, utcnow(), Role, User
...
Cohesion: 0.14
Nodes (16): AdminPage(), conversations, Home(), sources, ChatPage(), ChatWorkbench(), FeedbackPage(), KnowledgePage() (+8 more)
...
Cohesion: 0.24
Nodes (6): AnswerResult, ChatRequest, AnswerResult, ChatRequest, AnswerRouter, TemplateStrategy
...
- **Why does `ChatService` connect `10. 后续扩展边界` to `Community 32`, `Community 5`?**
  _High betweenness centrality (0.033) - this node is a cross-community bridge._
- **Are the 23 inferred relationships involving `CurrentUser` (e.g. with `Any` and `CurrentUser`) actually correct?**
  _`CurrentUser` has 23 INFERRED edges - model-reasoned connections that need verification._
- **Are the 20 inferred relationships involving `ChatRequest` (e.g. with `CurrentUser` and `ChatRequest`) actually correct?**
  _`ChatRequest` has 20 INFERRED edges - model-reasoned connections that need verification._
- **Are the 10 inferred relationships involving `ChatService` (e.g. with `ChatRequest` and `ChatResponse`) actually correct?**
  _`ChatService` has 10 INFERRED edges - model-reasoned connections that need verification._
- **Are the 14 inferred relationships involving `ChatResponse` (e.g. with `ChatRequest` and `CurrentUser`) actually correct?**
  _`ChatResponse` has 14 INFERRED edges - model-reasoned connections that need verification._